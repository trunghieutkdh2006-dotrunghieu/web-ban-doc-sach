(() => {
    ...
    // Lấy avatar từ localStorage theo userId
    function getAvatarFromLocalStorage(userId, userName) {
    ...
    // Lấy avatar từ server API
    async function getAvatarFromServer(userId) {
        if (!userId) return null;
        ...
        try {
            ...
            console.error('Lỗi lấy avatar từ server:', err);
        }
        return null;
    }
    ...
    // Hàm chính để lấy avatar cho review
    async function getAvatarForReviewAsync(userName, reviewUserId) {
        // Lấy userId từ review
        let userId = null;
        if (reviewUserId) {
            if (typeof reviewUserId === 'object') {
                userId = reviewUserId._id || reviewUserId.id;
            } else {
                userId = reviewUserId;
            }
        }
        ...
        avatarUrl = getAvatarFromLocalStorage(userId, userName);
        
        // Nếu không có, thử lấy từ server
        if (!avatarUrl && userId) {
            avatarUrl = await getAvatarFromServer(userId);
        }
        
        let result;
        if (avatarUrl) {
            result = `<img src="${avatarUrl}" style="width: 42px; height: 42px; border-radius: 50%; object-fit: cover;" onerror="this.src='https://ui-avatars.com/api/?background=1D3557&color=fff&size=42&length=1&name=${userName?.charAt(0) || 'U'}&bold=true'">`;
        } else {
            const initial = userName ? userName.charAt(0).toUpperCase() : 'U';
            result = `<div style="width: 42px; height: 42px; border-radius: 50%; background: #1D3557; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 18px;">${initial}</div>`;
        }
    ...
    async function displayReviews() {
        ...
        // Hiển thị loading
        container.innerHTML = `<div class="loading"><i class="fas fa-spinner fa-spin"></i><p>Đang tải...</p></div>`;
        
        // Xử lý từng review và lấy avatar
        const reviewsHtml = [];
        for (const r of pageData) {
            const displayName = r.userName || "Độc giả";
            const avatarHtml = await getAvatarForReviewAsync(displayName, r.userId);
